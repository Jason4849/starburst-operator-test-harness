...
var _ = ginkgo.Describe("Default Operator Tests:", func() {
	...
	ginkgo.It("starbursthives.charts.starburstdata.com CRD exists", func() {
		apiextensions, err := clientset.NewForConfig(config)
		Expect(err).NotTo(HaveOccurred())

		// Make sure the CRD exists
		_, err = apiextensions.ApiextensionsV1().CustomResourceDefinitions().Get(context.TODO(), "starbursthives.charts.starburstdata.com", metav1.GetOptions{})
